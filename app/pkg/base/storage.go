...
// GenUploadSingle .
func GenUploadSingle(filename string, expire int, respChan chan *proto.GenUpload,
	...
	single := fmt.Sprintf("/api/storage/rpc/upload?%s", queryString)
	multi := fmt.Sprintf("/api/storage/rpc/upload/multi?%s", queryString)
	merge := fmt.Sprintf("/api/storage/rpc/upload/merge?%s", queryString)
	respChan <- &proto.GenUpload{
		Uid: uidStr,
		Url: &proto.UrlResult{
			Single: single,
			Multi: &proto.MultiUrlResult{
				Merge:  merge,
				Upload: multi,
			},
		},
...
func GenDownloadSingle(meta models.MetaDataInfo, expire string, respChan chan *proto.GenDownload,
	wg *sync.WaitGroup) {
	defer wg.Done()
	uid := meta.UID
	...
	url := fmt.Sprintf("/api/storage/rpc/download?%s", queryString)
	info := proto.GenDownload{
